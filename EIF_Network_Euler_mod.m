function [Se, Si] = EIF_Network_Euler_mod(Ne,Ni,VT,EL,time,dt,D, taum, taux, ...
    taue, taui, Vth, Vre, Sex, Six,Jex,Jee,Jei,Jix,Jie,Jii)
...
for tt = 1:length(time)-1
    ...
    Iex=Iex+dt*(-Iex+Jex*Sex(:,tt))/taux;      
    Iee=Iee+dt*(-Iee+Jee*Se(:,tt))/taue;
    Iei=Iei+dt*(-Iei+Jei*Si(:,tt))/taui;   
    Iix=Iix+dt*(-Iix+Jix*Six(:,tt))/taux;    
